...
class Trainer:
    ...
    def __init__(self, model: GomokuNet, lr: float = 0.001, weight_decay: float = 1e-4,
                 device: str = None):
        self.model = model
    ...
    def train_step(self, batch: List[SelfPlayData]) -> Dict[str, float]:
        ...
        # Metrics
        with torch.no_grad():
            policy_acc = (pred_policies.argmax(dim=1) == policies.argmax(dim=1)).float().mean()
    ...
    def train_epoch(self, data_buffer: DataBuffer, batch_size: int = 512, 
                   steps_per_epoch: int = 1000) -> Dict[str, float]:
        """Train for one epoch"""
        self.model.train()
        epoch_metrics = []
