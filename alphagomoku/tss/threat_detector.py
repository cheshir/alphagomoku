...
class ThreatDetector:
    ...
    def detect_threats(self, position: Position, player: int) -> List[Tuple[int, int, ThreatType]]:
    ...
    def _analyze_cell_threats(self, position: Position, row: int, col: int, player: int) -> List[ThreatType]:
    ...
    def _get_line_pattern(self, board: np.ndarray, row: int, col: int, 
                         dr: int, dc: int, player: int) -> str:
        ...
        back_stones = []
        empty_count = 0
        while (0 <= r < self.board_size and 0 <= c < self.board_size and len(back_stones) < 5):
            if board[r, c] == player:
        ...
        empty_count = 0
        forward_count = 0
        while (0 <= r < self.board_size and 0 <= c < self.board_size and forward_count < 5):
            if board[r, c] == player:
                pattern += 'X'
            elif board[r, c] == -player:
                break
    ...
    def _classify_pattern(self, pattern: str) -> ThreatType:
        ...
        for bp in broken_four_patterns:
            if bp in pattern:
                ...
                if '.' in pattern and (pattern.startswith('.') or pattern.endswith('.')):
    ...
    def is_winning_threat(self, position: Position, row: int, col: int, player: int) -> bool:
        """Check if move creates an immediate winning threat."""
        threats = self._analyze_cell_threats(position, row, col, player)
        return (ThreatType.OPEN_FOUR in threats or 
                ThreatType.DOUBLE_FOUR in threats or
                ThreatType.DOUBLE_THREE in threats)
    ...
    def _creates_five_in_row(self, board: np.ndarray, row: int, col: int, player: int) -> bool:
