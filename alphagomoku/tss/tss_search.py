...
class TSSSearcher:
    ...
    def _search_forced_win(self, position: Position, depth: int, player: int) -> Optional[Tuple[int, int]]:
    ...
    def _opponent_cannot_defend(self, position: Position, depth: int, opponent: int) -> bool:
    ...
    def _get_prioritized_moves(self, position: Position, player: int) -> List[Tuple[int, int]]:
        ...
        def threat_priority(move):
            row, col = move
            threats = self.threat_detector._analyze_cell_threats(position, row, col, player)
            priority = 0
